  ...
  for (const day of copy.responses.postseason.dates) {
    for (const g of day.games) {
      ...
      if (g.gameType === "D" && dsAway) {
        ...
        const host = [115, 116, 136, 141, 143, 158].includes(g.teams.home.team.id)
          ? "away"
          : "home";
...
test("a bracket just set: twelve real clubs, division series opponents still placeholders", () => {
  const snap = build(
    rewind(SEASON_2025, "2025-09-30T00:00:00Z", { unsetRounds: ["CS", "WS"], dsAway: true }),
  ...
  assert.deepEqual(snap.series.AL_WC1, {
    winsA: 0,
    winsB: 0,
...
test("halfway: division series under way, the next game named, later rounds waiting", () => {
  const snap = build(
    rewind(SEASON_2025, "2025-10-08T12:00:00Z", { unsetRounds: ["CS", "WS"] }),
