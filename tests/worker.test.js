...
const ctx = vm.createContext({
  AbortSignal, Date, Intl, JSON, Map, Math, Promise, Request, Response, Set, URL, console
});
...
const EVENING = JSON.parse(fs.readFileSync(path.join(__dirname, "fixtures/2026-09-24-evening.json"), "utf8"));
...
function fakeMlb({ status = 200 } = {}){
  ...
  const fetchImpl = async (url, init) => {
    calls.push({ url, init });
    const kind = url.includes("/standings") ? "standings" : url.includes("/postseason") ? "postseason" : "schedule";
...
function worker(opts = {}){
  ...
  return { w, mlb, tick: ms => { now += ms; } };
}
const rpc = (w, body, { env = {}, path: p = "/mcp" } = {}) => w.fetch(new Request(`https://mlb-live.example${p}`, {
...
test("the handshake claude.ai makes when the connector is added", async () => {
  const { w } = worker();
  const res = await rpc(w, { jsonrpc: "2.0", id: 1, method: "initialize",
...
test("an unknown protocol version gets the newest one back", async () => {
  const { w } = worker();
  const { result } = await (await rpc(w, { jsonrpc: "2.0", id: 1, method: "initialize", params: { protocolVersion: "1999-01-01" } })).json();
  assert.equal(result.protocolVersion, "2025-06-18");
});
...
test("get_snapshot answers with the snapshot, structured and as text", async () => {
  ...
  const expected = JSON.parse(JSON.stringify(MLBSnapshot.buildSnapshot(EVENING.responses, { season: 2026, now: NOW })));
...
test("bad arguments are refused before anything is fetched", async () => {
  const { w, mlb } = worker();
  for(const args of [{ season: "2026" }, { season: 2026.5 }, { season: 1800 }, { season: 2026, team: "NYY" }]){
...
test("malformed requests get JSON-RPC errors", async () => {
  ...
  assert.equal((await (await rpc(w, { jsonrpc: "2.0", id: 1, method: "resources/list" })).json()).error.code, -32601);
  assert.deepEqual((await (await rpc(w, { jsonrpc: "2.0", id: 9, method: "ping" })).json()).result, {});
  const big = await rpc(w, JSON.stringify({ jsonrpc: "2.0", id: 1, method: "ping", pad: "x".repeat(70000) }));
  assert.equal(big.status, 413);
});

test("a batch gets one answer per request", async () => {
  const { w } = worker();
  const res = await rpc(w, [{ jsonrpc: "2.0", id: 1, method: "ping" }, { jsonrpc: "2.0", method: "notifications/initialized" }]);
  assert.deepEqual((await res.json()).map(r => r.id), [1]);
});
...
test("GET on the MCP endpoint is refused; /snapshot answers plain HTTP", async () => {
  ...
  assert.equal((await w.fetch(new Request("https://mlb-live.example/snapshot?season=abc"), {})).status, 400);
});
...
test("with CONNECTOR_KEY set, only the keyed paths answer", async () => {
  ...
  assert.equal((await rpc(w, { jsonrpc: "2.0", id: 1, method: "ping" }, { env, path: "/mcp/s3cret" })).status, 200);
  assert.equal((await w.fetch(new Request("https://mlb-live.example/"), env)).status, 404);
  assert.equal((await w.fetch(new Request("https://mlb-live.example/snapshot/s3cret?season=2026"), env)).status, 200);
});
