/* Deploys the connector through Cloudflare's REST API, without wrangler.
   ...
   Anywhere else, set CLOUDFLARE_API_TOKEN and this sends it itself.
     ...
     CLOUDFLARE_ACCOUNT_ID=<id> node worker/deploy.mjs      (npm run deploy:api)

   Three calls: upload worker/dist/worker.mjs as the script, switch on its
   workers.dev route, and read the account's workers.dev subdomain to print
   the connector's URL. The name and compatibility date come from
   worker/wrangler.toml, so both ways of deploying produce the same Worker. */
